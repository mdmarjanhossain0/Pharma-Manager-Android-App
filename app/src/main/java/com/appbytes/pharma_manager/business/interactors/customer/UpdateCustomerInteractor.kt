...
import com.appbytes.pharma_manager.business.datasource.network.customer.network_response.toCustomer
        ...
        try {
            ...
            when (e) {
                is HttpException -> {
                    ...
                    Log.d(com.appbytes.pharma_manager.business.interactors.account.TAG, body)
                    val gson = GsonBuilder().create()
                    val errorEntity : GenericResponse = Gson().fromJson(body, GenericResponse::class.java)
                    Log.d(com.appbytes.pharma_manager.business.interactors.account.TAG, errorEntity.toString() + " ")
                    when (e.code()) {
                        400 ->
                            Log.d(com.appbytes.pharma_manager.business.interactors.account.TAG, "400 Bad Request " + e.response()?.errorBody().toString())
                        500 ->
                            Log.d(com.appbytes.pharma_manager.business.interactors.account.TAG, "500 Bad Request " + e.response()?.errorBody().toString())
                        401 ->
                            Log.d(com.appbytes.pharma_manager.business.interactors.account.TAG, "401 Bad Request " + e.response()?.errorBody().toString())

                        404 -> {
                            Log.d(com.appbytes.pharma_manager.business.interactors.account.TAG, "404 Bad Request " + e.response()?.errorBody().toString())
