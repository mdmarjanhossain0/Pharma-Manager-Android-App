...
/*
    ...
    private var isRViewScrolling: Boolean
...
///////////// Clickable //////////////////// *//*
    ...
    private fun isIndependentViewClicked(motionEvent: MotionEvent): Boolean {
        for (i in independentViews.indices) {
            if (touchedView != null) {
                ...
                touchedView.findViewById(independentViews[i]).getGlobalVisibleRect(rect)
    ...
    private fun getOptionViewID(motionEvent: MotionEvent): Int {
        for (i in optionViews.indices) {
            if (touchedView != null) {
                ...
                val x = motionEvent.rawX.toInt()
                val y = motionEvent.rawY.toInt()
                touchedView.findViewById(optionViews[i]).getGlobalVisibleRect(rect)
                if (rect.contains(x, y)) {
    ...
    fun closeVisibleBG(mSwipeCloseListener: OnSwipeListener?) {
        ...
        val translateAnimator: ObjectAnimator = ObjectAnimator.ofFloat<Any>(
            bgVisibleView,
            View.TRANSLATION_X, 0f
        )
        translateAnimator.duration = ANIMATION_CLOSE
        translateAnimator.addListener(object : AnimatorListener() {
    ...
    private fun animateFG(
        ...
        if (animateType == Animation.OPEN) {
            translateAnimator =
                ObjectAnimator.ofFloat<Any>(fgView, View.TRANSLATION_X, -bgWidth.toFloat())
            translateAnimator.duration = duration
            translateAnimator.interpolator = DecelerateInterpolator(1.5f)
            translateAnimator.start()
            animateFadeViews(downView, 0f, duration)
...
/*if (animateType == Animation.CLOSE)*//*
 {
            translateAnimator = ObjectAnimator.ofFloat<Any>(fgView, View.TRANSLATION_X, 0f)
            translateAnimator.duration = duration
            translateAnimator.interpolator = DecelerateInterpolator(1.5f)
            translateAnimator.start()
            animateFadeViews(downView, 1f, duration)
...
if (!isFgSwiping && Math.abs(deltaX) > touchSlop && Math.abs(deltaY) < Math.abs(
                ...
                if (swipeable && isFgSwiping && !unSwipeableRows.contains(touchedPosition)) {
                    ...
                    if (deltaX < touchSlop && !bgVisible) {
                        ...
                        fgView.setTranslationX(if (Math.abs(translateAmount) > bgWidth) -bgWidth else translateAmount)
                        ...
                        if (fadeViews != null) {
                            for (viewID in fadeViews!!) {
                                touchedView.findViewById(viewID)
                        ...
                        if (bgVisible) {
                            ...
                            fgView.setTranslationX(if (translateAmount > 0) 0 else translateAmount)
