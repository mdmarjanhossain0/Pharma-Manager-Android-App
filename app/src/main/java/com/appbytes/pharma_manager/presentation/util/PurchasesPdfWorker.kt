...
import com.appbytes.pharma_manager.business.datasource.cache.account.toAccount
...
@HiltWorker
class PurchasesPdfWorker @AssistedInject
constructor(
    @Assisted
    context: Context,
    @Assisted
    ...
    override suspend fun doWork(): Result {
        ...
        if (pk > 0) {
            val order = purchasesDao.getPurchasesOrder(pk).toPurchasesOder()
    ...
    private fun createPDF(order : PurchasesOrder, account : Account) : String {
...
//        table.addCell(Cell().add(Paragraph("Test")))
        ...
        if (order.vendor == null) {
            val customerName = Paragraph("Customer Name : Walk-In Supplier").setTextAlignment(
                TextAlignment.LEFT)
            ...
            val customerName = Paragraph("Customer Name : " + order.company).setTextAlignment(
                TextAlignment.LEFT)
            val cell = Cell(1, 4).setBorder(Border.NO_BORDER)
            cell.add(customerName)
            dataTable.addCell(cell)
...
//        orderInfoTable.addCell(Cell().add(Paragraph("")))
        ...
        for (item in order.purchases_order_medicines!!) {
            ...
            dataTable.addCell(Cell().add(Paragraph((item.purchase_price.toString() + " ৳ ").toString()))).setTextAlignment(
        ...
        // Data Table Total
        val itemCount = Paragraph("Item : " + order.purchases_order_medicines?.size).setTextAlignment(
            TextAlignment.LEFT)
