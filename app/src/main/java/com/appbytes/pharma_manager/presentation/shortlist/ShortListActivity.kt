package com.appbytes.pharma_manager.presentation.shortlist
...
import com.appbytes.pharma_manager.presentation.inventory.InventoryFragmentDirections
...
import kotlinx.android.synthetic.main.activity_short_list.*
import kotlinx.android.synthetic.main.layout_drawer_menu.*

@AndroidEntryPoint
class ShortListActivity : BaseActivity(), View.OnClickListener {
    ...
    override fun displayProgressBar(isLoading: Boolean) {
        if(isLoading){
            short_list_progress_bar.visibility = View.VISIBLE
        }
        else{
            short_list_progress_bar.visibility = View.GONE
        }
    }
    ...
    private fun setUpUI() {
        val navHostFragment =
            supportFragmentManager.findFragmentById(R.id.short_list_fragment_container) as NavHostFragment
    ...
    override fun onClick(v: View?) {
        when (v!!.id) {
            ...
            R.id.navShortListTvId -> {
                drawerLayout!!.closeDrawer(navigationView, true)
                Toast.makeText(this, "You are in Short List", Toast.LENGTH_SHORT).show()
//                this.startActivity(Intent(this, SalesActivity::class.java))
            }
