...
import com.devscore.digital_pharmacy.business.interactors.supplier.GetSupplierDetailsInteractor
import com.devscore.digital_pharmacy.business.interactors.supplier.SupplierPreviousOrderInteractor
...
constructor(
    ...
    private val supplierDetailsInteractor: GetSupplierDetailsInteractor,
    private val supplierPreviousOrderInteractor: SupplierPreviousOrderInteractor
) : ViewModel() {

    private val TAG: String = "AppDebug"

    val state: MutableLiveData<SupplierDetailsState> = MutableLiveData(SupplierDetailsState())
    ...
    fun onTriggerEvent(event: SupplierDetailsEvents) {
        when (event) {
            is SupplierDetailsEvents.SearchOrders -> {
                search(event.pk)
            }

            is SupplierDetailsEvents.GetDetails -> {
                getDetails(event.pk)
            }

            is SupplierDetailsEvents.NextPage -> {
                incrementPageNumber()
                search(event.pk)
            }

            is SupplierDetailsEvents.UpdateQuery -> {
                onUpdateQuery(event.query)
            }

            is SupplierDetailsEvents.Error -> {
                appendToMessageQueue(event.stateMessage)
            }
            is SupplierDetailsEvents.OnRemoveHeadFromQueue -> {
                removeHeadFromQueue()
            }
        }
    }
    ...
    private fun getDetails(pk: Int) {
        state.value?.let { state ->
            ...
            supplierDetailsInteractor.execute(
...
//                this.state.value = state.copy(isLoading = false)
                ...
                dataState.data?.let { details ->
                    this.state.value = state.copy(supplier = details)
                }
    ...
    private fun search(pk: Int) {
        ...
        state.value?.let { state ->
            supplierPreviousOrderInteractor.execute(
                authToken = sessionManager.state.value?.authToken,
                pk = pk,
                page = state.page
            ).onEach { dataState ->
