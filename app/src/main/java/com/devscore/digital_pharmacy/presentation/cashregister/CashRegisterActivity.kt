...
import com.devscore.digital_pharmacy.presentation.inventory.InventoryFragmentDirections
...
@AndroidEntryPoint
class CashRegisterActivity : BaseActivity(), View.OnClickListener {
    ...
    override fun displayProgressBar(isLoading: Boolean) {
        if(isLoading){
            cashRegisterProgressBar.visibility = View.VISIBLE
        }
        else{
            cashRegisterProgressBar.visibility = View.GONE
        }
    }
    ...
    private fun setUpUI() {
        val navHostFragment =
            supportFragmentManager.findFragmentById(R.id.cashRegisterContainer) as NavHostFragment
    ...
    override fun onClick(v: View?) {
        when (v!!.id) {
            ...
            R.id.navCashReceiveTvId -> {
                drawerLayout!!.closeDrawer(navigationView, true)
                Toast.makeText(this, "You are in Cash Register", Toast.LENGTH_SHORT).show()
//                this.startActivity(Intent(this, CashRecordActivity::class.java))
    ...
    fun navigateReceiveToCustomerFragment() {
        navController.navigate(R.id.action_cashVendorFragment_to_salesCustomerListFragment2)
    }

    fun navigatePaymentToSupplierFragment() {
        navController.navigate(R.id.action_cashVendorFragment_to_cashRegisterSupplierListFragment)
    }
}